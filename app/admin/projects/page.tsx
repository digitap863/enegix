  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="font-exo font-bold text-2xl text-slate-800">Manage Projects</h2>
          <p className="text-sm text-slate-500">Track and publish engineering, installation, and supply projects.</p>
        </div>
        <Link
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-4 flex flex-col md:flex-row gap-4 items-center justify-between shadow-xs">
        <div className="w-full md:max-w-md relative">
          <input
            type="text"
            placeholder="Search projects by code, title or client..."
            value={searchTerm}
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-xs overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-400">
            ...
            <p className="text-xs text-slate-400 mt-1">Try refining your filter conditions or publish a new project.</p>
          </div>
        ) : (
                    ...
                    <td className="px-6 py-4 max-w-xs">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="truncate">
                          ...
                          <span className="text-[10px] text-slate-450 truncate block">Code: {project.projectCode}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center px-2 py-0.5 rounded-xs text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-700">
                        {project.category}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-slate-600 font-medium">{project.client}</td>
                    <td className="px-6 py-4 text-slate-500 text-xs">{project.location}</td>
                    <td className="px-6 py-4 text-slate-500 text-xs font-mono">{project.year}</td>
                    <td className="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                      <Link
                        href={`/admin/projects/new?edit=${project._id}`}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full border border-slate-200 shadow-2xl space-y-4 animate-fade-in-up">
            <div className="flex items-center gap-3 text-red-600">
              ...
              <h4 className="font-exo font-bold text-lg text-slate-800">Delete Project?</h4>
            </div>
            <p className="text-sm text-slate-500 leading-relaxed">
              Are you sure you want to delete this project? This will permanently delete the project listing.
            </p>
